...
class BlueCircleTracker:
    ...
    def __init__(self, circle_diameter=0.05, intrinsic_file=None):
        ...
        if intrinsic_file is None:
            ...
            intrinsic_file = os.path.join(script_dir, 'config_data/camera_intrinsics_right.yaml')
    ...
    def detect_blue_circle(self, frame):
        ...
        if circularity > 0.5:
            ...
            if len(largest_contour) >= 5:
                ...
                # 放宽拟合误差要求，允许更大的形变
                if mean_error < radius * 0.25 and std_error < radius * 0.35:  # 误差在25%以内
                    return True, center, radius, mask
    ...
    def estimate_pose_from_circle(self, center, radius_px):
        ...
        假设圆盘平面与XY平面平行（法向量指向Z轴）
        
        Returns
        -------
        T_circle_cam : np.ndarray (4x4)
            圆盘在相机坐标系下的位姿矩阵
        """
        fx = self.K[0, 0]
        fy = self.K[1, 1]
        ...
        # 构建位姿矩阵
        # 假设圆盘平面平行于相机XY平面，法向量指向Z轴
        T_circle_cam = np.eye(4)
        T_circle_cam[:3, :3] = np.eye(3)  # 无旋转（与相机坐标系对齐）
        T_circle_cam[:3, 3] = pos
        ...
        try:
            while True:
                ...
                if success:
                    ...
                    # 估算3D位姿
                    T_circle_cam = self.estimate_pose_from_circle(center, radius_px)
                    pos_cam = T_circle_cam[:3, 3]
                    pos_cam_mm = pos_cam * 1000
                    
                    # 绘制3D坐标轴
                    axis_length = 0.03  # 30mm
                    ...
                    rvec, _ = cv2.Rodrigues(T_circle_cam[:3, :3])
                    axis_2d, _ = cv2.projectPoints(axis_points, rvec, T_circle_cam[:3, 3], self.K, self.dist)
                    ...
                    cv2.line(display, origin, pt_x, (0, 0, 255), 2)  # X - 红
                    cv2.line(display, origin, pt_y, (0, 255, 0), 2)  # Y - 绿
                    cv2.line(display, origin, pt_z, (255, 0, 0), 2)  # Z - 蓝
                    ...
                    if following and self.T_cam_gripper is not None:
                        ...
                        # 计算目标在Base坐标系下的位姿
                        # ^B T_circle = ^B T_G @ ^G T_C @ ^C T_circle
                        T_base_cam = T_gripper_base @ self.T_cam_gripper
                        T_circle_base = T_base_cam @ T_circle_cam
                        
                        pos_target_base = T_circle_base[:3, 3]
