...
本章对应 2.3 的“手眼标定设计思路/实现思路”，并与仓库实现一致：
...
实现中给出经验阈值（mm/deg），用于打印“优秀/一般/较差”。

工程建议：
- 采集姿态尽量覆盖不同距离、不同倾角、不同视野位置。
- PnP 的重投影误差过大（例如 >2px）应丢弃该样本。

## 5. 与数据集采集（2.3）的关系

- 有了 $T_{CB}$（Eye-to-Hand）后，可把相机坐标系下的目标点转换到基座系，实现视觉引导抓取。
- 若后续要把图像中的目标与末端轨迹做更严格对齐，需要把相机时间戳/帧序号也纳入数据集（见 `stage1_2_3_timesync.md`）。
