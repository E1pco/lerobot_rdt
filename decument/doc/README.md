...
- 运动学/IK 理论：`theory_kinematics_ik.md`
...
2) 2.1 SDK：先跑 `arm_keyboard_control.py`（关节级，验证方向/限位），再跑 `ik_keyboard_realtime.py`（末端级，验证 FK/IK）
3) 2.2 遥操作：跑 `joycon_ik_control_py.py`（形成可录制演示视频的稳定控制）
4) 2.3 标定：先相机内参，再手眼（必要时做一致性评估），最后用验证脚本做闭环演示
5) 2.3 采集：采 raw → raw→HDF5 → inspect 校验张量形状与抽样可视化
