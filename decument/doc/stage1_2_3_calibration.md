...
- **`cv2.calibrateHandEye(...)`**
  - **作用**：求解手眼方程 $AX=XB$。
  - **细节**：输入一系列的 $R\_{gripper}^{base}, t\_{gripper}^{base}$ 和 $R\_{target}^{cam}, t\_{target}^{cam}$，输出手眼变换矩阵（Eye-in-Hand 为 $T\_{cam}^{gripper}$，Eye-to-Hand 为 $T\_{cam}^{base}$）。常用算法为 `CALIB_HAND_EYE_TSAI` 或 `CALIB_HAND_EYE_PARK`。
